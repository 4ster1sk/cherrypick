...
- `커스텀 이모지`에서 여러 이모지를 한 번에 등록하는 기능이 폐지되었습니다.
...
- Feat: 배포 대상 제한 (Misaki-Project/cherrypick#40)
...
- Fix: `노트 삭제 예약`이 적용된 노트를 초안으로 저장하거나 게시 예약하면 지정한 삭제 예약 시간이 저장되지 않을 수 있음
...
- Fix: 한 줄로 긴 내용이 포함된 노트에서 `텍스트 소스 보기`를 사용하면 내용이 잘려서 표시될 수 있음
...
## 4.16.0
출시일: 2025/6/15<br>
기반 Misskey 버전: 2025.5.0<br>
Misskey의 전체 변경 사항을 확인하려면, [CHANGELOG.md#202550](CHANGELOG.md#202550) 문서를 참고하십시오.

### NOTE
- Misskey에서의 설정 기능 개편에 대응하는 과정에서 기존에 존재했던 일부 설정이 누락되거나, 기능이 정상적으로 작동하지 않거나, 토글이 작동하지 않는 등의 문제가 발생할 수 있습니다.
	이러한 문제를 발견하셨다면, [문제 보고](https://github.com/kokonect-link/cherrypick/issues/new?template=01_bug-report.yml)를 통해 버그를 리포트하실 수 있습니다.
- Misskey 측에서 채팅 기능이 새로 구현됨에 따라, 이 버전부터는 새로운 채팅 기능이 도입되며 이전 채팅 기능은 더 이상 이용할 수 없게 됩니다. 
	이전 대화 내역은 마이그레이션 과정에서 그대로 유지되지만, 이 과정에서 일부 대화 내역이 유실될 수도 있습니다.
	이 변경으로 인해 이전 버전의 CherryPick 또는 채팅 기능을 사용하는 클라이언트와는 더 이상 호환되지 않으며 채팅을 주고 받을 수 없습니다.
	또한, 채팅 기능이 서버간 연합을 지원하지 않게 됩니다. 향후 업데이트를 통해 다시 연합을 지원할 예정입니다.

### Client
- Feat: 노트 본문 사이를 띄워서 표시할 수 있음
  - 아바타 장식이 잘리는 등의 문제를 해결하기 위해 본문 왼쪽에 여백을 추가하여 Misskey와 동일한 디자인을 사용할 수 있습니다.
- Enhance: (Friendly) `스크롤 시 요소 표시` 기능에 의해 숨기도록 설정하지 않은 요소는 스크롤하면 반투명하게 표시되도록 변경됨
- Enhance: 미디어를 첨부했을 때만 `우클릭 방지`를 토글할 수 있음
  - `우클릭 방지`는 미디어에만 적용되는 기능이며 노트 본문에는 적용되지 않습니다. 따라서 사용자가 혼란을 느낄 수 있다고 판단되어 수정되었습니다.
- Enhance: 미디어를 포함한 노트를 게시할 때, 미디어 첨부가 완료될 때까지 노트를 게시할 수 없음
- Enhance: 진동 지원 기기에서 진동 기능을 활성화 했을 때, 진동을 테스트할 수 있음
...
- Fix: `답글 대상 노트 보이기`를 끄면 일부 조건에서 노트가 표시되지 않을 수 있음
- Fix: `답글 대상 노트 보이기`를 끄면 사용자 페이지의 타임라인에서 일부 요소가 잘못 표시될 수 있음
- Fix: `둘러보기` 탭으로 이동했을 떄 하단 네비게이션 바의 `둘러보기` 버튼이 활성화된 것으로 표시되지 않음
- Fix: `버블 타임라인`을 활성화해도 타임라인 목록에 표시되지 않을 수 있음
- Fix: `일정 시간이 지난 후 접속했을 때 환영 메시지 표시`를 사용할 때 표시되는 토스트 알림이 프로필 아이콘 모앙을 제대로 반영하지 않음
- Fix: 로그인하지 않은 사용자가 로컬/글로벌 타임라인을 볼 수 없을 수 있음 (kokonect-link/cherrypick#574)
- Fix: `답글 대상 노트 보이기`가 꺼져있으면 `답글 대상 노트를 반투명하게 표시` 설정을 변경할 수 없음
- Fix: 일부 기기에서 검색 블록에 입력한 내용을 `x` 버튼을 클릭해 지울 때 터치 효과가 표시될 수 있음
- Fix: 회원 가입 시 비밀번호 입력란에서 `Caps Lock`이 켜져있을 때 표시되는 아이콘이 잘못된 디자인으로 표시될 수 있음
- Fix: `답글 대상 노트 보이기`가 꺼져있으면 CW가 포함된 노트의 답글 대상이 `더 보기`를 눌러야 표시됨
- Fix: 모바일 환경에서 게시 양식의 게시 버튼이 잘못된 디자인으로 표시될 수 있음
- Fix: `키보드 단축키` 도움말이 메뉴의 드롭다운 메뉴에 추가되지 않음
- Fix: 안테나 편집 페이지에서 헤더가 중복으로 표시될 수 있음
- Fix: 스크롤 시 헤더 위쪽에 표시되는 배너가 자연스럽게 숨겨지지 않음
- Fix: iPad 환경에서 `움직이는 이미지를 자동으로 재생하지 않음` 설정이 자동으로 `일정 시간이 지나면 멈춤`으로 설정되어 있지 않음
...
Misskey의 전체 변경 사항을 확인하려면, [CHANGELOG.md#202521](CHANGELOG.md#202521) 문서를 참고하십시오.
...
- Enhance: 사용자 팝업에 계정 배지를 표시함
  - 이제 `관리자` 계정, `팔로우를 수동으로 승인`으로 설정한 사용자, `봇` 계정, `프록시` 계정을 쉽게 확인할 수 있습니다.
- Enhance: 사용자 팝업에서 `노트`, `팔로잉`, `팔로워`를 눌러 해당 페이지로 이동할 수 있음
- Enhance: 타임라인에서 답글 대상 노트를 숨길 수 있음
  - `답글 대상 노트 보이기`를 비활성화 하면 해당 노트가 누구에게 답글을 작성한 노트인지 알 수 있도록 사용자 이름을 표시합니다.
- Enhance: 게시 양식 개선
  - 노트를 게시할 때 사용할 수 있는 설정이 포함되어 있습니다.
  - 노트를 초안으로 저장하려면, 게시 버튼 옆의 드롭다운 메뉴를 눌러 초안으로 저장하도록 설정할 수 있습니다.
    - 게시 양식을 닫으면 서버에 저장하지 않고, 기존과 같이 브라우저에만 임시로 저장됩니다.
  - `우클릭 방지` 설정도 이 드롭다운 메뉴로 이동되었습니다.
...
- Fix: 사용자 제안 도중 `@`를 입력하면 제안 결과가 사라지지 않을 수 있음 (misskey-dev/misskey#14385)
- Fix: 답글 노트의 일부 요소가 페이지를 중복으로 이동시킬 수 있음
- Fix: 초안으로 저장한 노트를 불러오면 `노트 삭제 예약` 옵션의 설정과 상관없이 게시 양식에서 `노트 삭제 예약` 섹션이 표시될 수 있음

### Server
- Enhance: 알림 개선
  - 팔로우 요청이 있을 때와 팔로우가 수락됐을 때, 해당 사용자의 `username`을 표시함
- Fix: 그룹에 초대했을 때 일부 알림이 잘못 표시될 수 있음
- Fix: 알림 제목이 길면 내용이 잘려서 보일 수 있음
...
- Enhance: 비밀번호 입력 시 `Caps Lock`이 켜져있으면 아이콘을 표시함
