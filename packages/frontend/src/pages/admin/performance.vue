		...
		<SearchMarker path="/admin/performance" :label="i18n.ts.performance" :keywords="['performance']" icon="ti ti-bolt">
			<div class="_gaps">
				...
				<SearchMarker>
					<MkFolder :defaultOpen="true">
						<template #icon><SearchIcon><i class="ti ti-bolt"></i></SearchIcon></template>
						<template #label><SearchLabel>Misskey® Reactions Boost Technology™ (RBT)</SearchLabel><span class="_beta" style="vertical-align: middle;">{{ i18n.ts.beta }}</span></template>
...
function onChange_doNotSendNotificationEmailsForAbuseReport(value: boolean) {
	os.apiWithDialog('admin/update-meta', {
		doNotSendNotificationEmailsForAbuseReport: value,
	}).then(() => {
		fetchInstance(true);
	});
}
