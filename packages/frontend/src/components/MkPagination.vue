		...
		>
			...
			<div v-else key="_root_" class="_gaps">
				...
				<div v-if="direction === 'down' || direction === 'both'" v-show="downButtonVisible">
					<MkButton v-if="!downButtonLoading" v-appear="prefer.s.enableInfiniteScroll ? paginator.fetchOlder : null" :class="$style.more" primary rounded @click="downButtonClick">
						{{ i18n.ts.loadMore }}
					</MkButton>
					<MkLoading v-else/>
				</div>
