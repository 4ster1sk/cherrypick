...
<div :class="[$style.root, { '_forceShrinkSpacer': deviceKind === 'smartphone' }]">
	...
	<div :class="$style.nonTitlebarArea">
		...
		<div v-if="showWidgetsSide && !pageMetadata?.needWideArea && prefer.s.enableWidgetsArea" :class="$style.widgets">
			<XWidgets/>
		</div>
...
onMounted(() => {
	if (!showWidgetsSide.value) {
		window.addEventListener('resize', () => {
			if (window.innerWidth >= DESKTOP_THRESHOLD) showWidgetsSide.value = true;
		}, { passive: true });
	}
});
