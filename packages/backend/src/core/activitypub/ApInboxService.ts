...
export class ApInboxService {
	...
	private async undoInvite(actor: MiRemoteUser, activity: IInvite): Promise<string> {
		...
		const roomObject = activity.object;
		if (typeof roomObject === 'string') return this.undoInviteGame(actor, activity);

		if (isGame(roomObject)) return this.undoInviteGame(actor, activity);
	...
	private async invite(actor: MiRemoteUser, activity: IInvite, resolver?: Resolver): Promise<string> {
		...
		const roomObject = activity.object;
		if (typeof roomObject === 'string') return this.inviteGame(actor, activity, resolver);

		if (isGame(roomObject)) return this.inviteGame(actor, activity, resolver);
